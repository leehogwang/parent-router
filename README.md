...
`/parent-stats` supports `--limit N|0`, `--date YYYY-MM-DD`, `--since YYYY-MM-DD`, `--until YYYY-MM-DD`, `--window Nd`, `--status ok|failed|dry-run`, `--profile parent|parent-no-opus`, `--mode plan|execute`, `--model haiku|sonnet|opus`, `--confidence high|medium|low`, `--format text|tsv|json`, `--fields a,b,c|core|debug`, `--reasons-only`, `--fail-if-empty`, `--summary-only`, `--show-paths`, `--sort newest|oldest`, and `--count-only`.
...
/parent-stats --window 7d --format tsv --fields debug --limit 0
```
...
claude -p '/parent-stats --window 7d --format json --fields debug --limit 0'
```
...
python3 scripts/parent_stats.py --window 7d --format tsv --fields debug --limit 0
```
...
Use `--count-only` when you want just the aggregate counts without headers or recent-run detail.

Use `--fields ...` with TSV or JSON output when you only need a subset of record columns.

Use `--fields core` when you want a stable minimal export of timestamp, model, mode, status, and confidence.
