---
description: Route a request to the appropriate Claude model, mode, and effort automatically
argument-hint: <goal>
allowed-tools: Bash
disable-model-invocation: true
---
You are handling a user-invoked `/parent` command.
...
3. Run:
   `ROOT="$PWD"; while [ ! -f "$ROOT/scripts/parent.py" ] && [ "$ROOT" != "/" ]; do ROOT="$(dirname "$ROOT")"; done; python3 "$ROOT/scripts/parent.py" --session-id "${CLAUDE_SESSION_ID}" --command-name "/parent" <<'__PARENTS_ARGS__'
$ARGUMENTS
__PARENTS_ARGS__`
