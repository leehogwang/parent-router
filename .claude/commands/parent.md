...
3. Run:
   `ROOT="$PWD"; while [ ! -f "$ROOT/scripts/parent.py" ] && [ "$ROOT" != "/" ]; do ROOT="$(dirname "$ROOT")"; done; python3 "$ROOT/scripts/parent.py" --session-id "${CLAUDE_SESSION_ID}" --command-name "/parent"`
