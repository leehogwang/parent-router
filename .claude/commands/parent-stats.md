...
argument-hint: [--limit N|0] [--date YYYY-MM-DD] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--window Nd] [--status ok|failed|dry-run] [--profile parent|parent-no-opus] [--mode plan|execute] [--model haiku|sonnet|opus] [--confidence high|medium|low] [--format text|tsv|json] [--reasons-only] [--fail-if-empty] [--summary-only] [--show-paths] [--sort newest|oldest]
allowed-tools: Bash
